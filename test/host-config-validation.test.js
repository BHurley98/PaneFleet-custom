...
import { waitForHttpServer } from './helpers/http.js';
...
async function rejectedHostConfig(config, { raw = false, environment = {} } = {}) {
  ...
  try {
    const [code] = await waitForChildExit(child, {
      timeoutMs: 3000,
      label: 'invalid host configuration'
    });
    return { code, output };
  } finally {
...
test('published service and host configuration examples pass production startup validation together', async () => {
  const fixtureDir = mkdtempSync(path.join(os.tmpdir(), 'panefleet-example-config-'));
  const hostConfigPath = path.join(fixtureDir, 'host-config.json');
  const port = await unusedLoopbackPort();
  let child;
  let output = '';
  try {
    copyFileSync(path.join(projectDir, 'services.example.json'), path.join(fixtureDir, 'services.json'));
    copyFileSync(path.join(projectDir, 'host-config.example.json'), hostConfigPath);
    ...
    await waitForHttpServer({
      ...
      child,
      output: () => output,
      label: 'published configuration example server'
    });
    assert.match(output, /PaneFleet listening/);
  } finally {
