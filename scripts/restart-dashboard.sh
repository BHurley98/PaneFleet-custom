#!/usr/bin/env bash
set -euo pipefail

UNIT="${ORCH_SYSTEMD_UNIT:-agent-orchestrator.service}"
WORKLOAD_UNIT="${ORCH_WORKLOAD_SYSTEMD_UNIT:-panefleet-workloads.service}"
HOST="${ORCH_HEALTH_HOST:-127.0.0.1}"
PORT="${ORCH_PORT:-8787}"
HEALTH_URL="http://$HOST:$PORT/healthz"
PROC_ROOT="${ORCH_PROC_ROOT:-/proc}"

[[ "$UNIT" =~ ^[A-Za-z0-9][A-Za-z0-9_.@-]{0,127}\.service$ ]] || { printf 'invalid ORCH_SYSTEMD_UNIT\n' >&2; exit 2; }
[[ "$WORKLOAD_UNIT" =~ ^[A-Za-z0-9][A-Za-z0-9_.@-]{0,127}\.service$ ]] || { printf 'invalid ORCH_WORKLOAD_SYSTEMD_UNIT\n' >&2; exit 2; }
[[ "$HOST" =~ ^[A-Za-z0-9.:-]+$ ]] || { printf 'invalid ORCH_HEALTH_HOST\n' >&2; exit 2; }
if [[ "$PROC_ROOT" != /proc ]]; then
  [[ "${ORCH_TEST_MODE:-0}" == 1 && "$PROC_ROOT" == /* && -d "$PROC_ROOT" && ! -L "$PROC_ROOT" ]] || {
    printf 'invalid ORCH_PROC_ROOT\n' >&2
    exit 2
  }
fi
if [[ -n "${XDG_RUNTIME_DIR:-}" ]]; then
  LOCK_DIR="$XDG_RUNTIME_DIR"
else
  TEMP_ROOT="${TMPDIR:-/tmp}"
  if [[ "$TEMP_ROOT" != /* || ! -d "$TEMP_ROOT" || -L "$TEMP_ROOT" ]]; then
    printf 'unsafe restart temporary directory\n' >&2
    exit 2
  fi
  LOCK_DIR="$TEMP_ROOT/panefleet-runtime-$UID"
  if [[ ! -e "$LOCK_DIR" ]]; then
    (umask 077; mkdir -- "$LOCK_DIR") || { printf 'could not create private restart lock directory\n' >&2; exit 2; }
  fi
  if [[ ! -d "$LOCK_DIR" || -L "$LOCK_DIR" || ! -O "$LOCK_DIR" ]]; then
    printf 'unsafe restart lock directory\n' >&2
    exit 2
  fi
  chmod 700 "$LOCK_DIR"
fi
if [[ "$LOCK_DIR" != /* || ! -d "$LOCK_DIR" || -L "$LOCK_DIR" || ! -O "$LOCK_DIR" ]]; then
  printf 'unsafe restart lock directory\n' >&2
  exit 2
fi
LOCK_FILE="$LOCK_DIR/agent-orchestrator-restart.lock"

[[ "$PORT" =~ ^[0-9]+$ ]] && (( PORT >= 1 && PORT <= 65535 )) || {
  printf 'invalid ORCH_PORT\n' >&2
  exit 2
}

command -v systemctl >/dev/null || { printf 'systemctl is required\n' >&2; exit 2; }
command -v curl >/dev/null || { printf 'curl is required\n' >&2; exit 2; }
command -v flock >/dev/null || { printf 'flock is required\n' >&2; exit 2; }
command -v ss >/dev/null || { printf 'ss is required\n' >&2; exit 2; }

workload_inventory() {
  if ! command -v tmux >/dev/null || ! tmux list-panes -a >/dev/null 2>&1; then
    printf 'server=absent\n'
    return 0
  fi
  local panes
  panes="$(tmux list-panes -a -F '#{session_name}|#{session_id}|#{session_created}|#{window_index}.#{pane_index}|#{pane_id}|#{pane_pid}|#{pane_start_command}' 2>/dev/null \
    | awk -F '|' '$1 != "agent-orchestrator" && $1 != "agent-orchestrator-watchdog"' \
    | LC_ALL=C sort)"
  if [[ -n "$panes" ]]; then
    printf '%s\n' "$panes"
  else
    printf 'server=present:no-workloads\n'
  fi
}

process_cgroup() {
  local pid="$1"
  [[ "$pid" =~ ^[1-9][0-9]*$ && -r "$PROC_ROOT/$pid/cgroup" ]] || return 1
  awk -F: '$1 == "0" { print $3; exit }' "$PROC_ROOT/$pid/cgroup"
}

assert_workload_cgroup_separate() {
  if ! command -v tmux >/dev/null || ! tmux list-panes -a >/dev/null 2>&1; then
    return 0
  fi
  local dashboard_pid tmux_pid dashboard_cgroup tmux_cgroup workload_service_cgroup
  dashboard_pid="$(systemctl --user show "$UNIT" -p MainPID --value 2>/dev/null || true)"
  tmux_pid="$(tmux display-message -p '#{pid}' 2>/dev/null || true)"
  dashboard_cgroup="$(process_cgroup "$dashboard_pid" 2>/dev/null || true)"
  tmux_cgroup="$(process_cgroup "$tmux_pid" 2>/dev/null || true)"
  workload_service_cgroup="$(systemctl --user show "$WORKLOAD_UNIT" -p ControlGroup --value 2>/dev/null || true)"
  if [[ -z "$dashboard_cgroup" || -z "$tmux_cgroup" || -z "$workload_service_cgroup" ]]; then
    printf 'could not prove workload tmux is isolated from PaneFleet; restart refused\n' >&2
    return 7
  fi
  if [[ "$dashboard_cgroup" == "$tmux_cgroup" ]]; then
    printf 'workload tmux shares the PaneFleet cgroup; run scripts/isolate-workload-tmux.sh before restart\n' >&2
    return 7
  fi
  if [[ "$tmux_cgroup" != "$workload_service_cgroup" ]]; then
    printf 'workload tmux is not owned by the dedicated workload service; run scripts/isolate-workload-tmux.sh before restart\n' >&2
    return 7
  fi
}

exec 9>"$LOCK_FILE"
flock -n 9 || { printf 'another PaneFleet restart is already running\n' >&2; exit 3; }

systemctl --user is-enabled --quiet "$UNIT" || {
  printf '%s is not installed and enabled; run scripts/install-control-plane.sh --migrate first\n' "$UNIT" >&2
  exit 4
}

before="$(workload_inventory)"
assert_workload_cgroup_separate || exit $?
systemctl --user restart "$UNIT"

ready=0
for _ in $(seq 1 80); do
  if systemctl --user is-active --quiet "$UNIT" && curl -fsS --max-time 2 "$HEALTH_URL" >/dev/null 2>&1; then
    ready=1
    break
  fi
  sleep 0.25
done

if [[ "$ready" != 1 ]]; then
  printf 'PaneFleet did not become healthy after restart\n' >&2
  systemctl --user --no-pager --full status "$UNIT" >&2 || true
  exit 5
fi

sleep 0.25
curl -fsS --max-time 2 "$HEALTH_URL" >/dev/null 2>&1 || {
  printf 'PaneFleet health was not stable across two samples\n' >&2
  exit 5
}

main_pid="$(systemctl --user show "$UNIT" -p MainPID --value)"
listener="$(ss -H -ltnp "sport = :$PORT" 2>/dev/null || true)"
if [[ ! "$main_pid" =~ ^[1-9][0-9]*$ || "$listener" != *"pid=$main_pid,"* ]]; then
  printf 'port %s listener is not owned by %s MainPID %s\n' "$PORT" "$UNIT" "${main_pid:-unknown}" >&2
  exit 5
fi

after="$(workload_inventory)"
if [[ "$before" != "$after" ]]; then
  printf 'workload tmux inventory changed during dashboard restart; investigate immediately\n' >&2
  diff -u <(printf '%s\n' "$before") <(printf '%s\n' "$after") >&2 || true
  exit 6
fi

printf 'PaneFleet healthy on %s (unit=%s pid=%s); workload tmux inventory unchanged\n' "$HEALTH_URL" "$UNIT" "$main_pid"
